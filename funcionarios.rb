...
response = HTTParty.get('http://localhost:3000/api/funcionarios')
funcionarios = response.parsed_response

funcionarios.each do |funcionario|
  puts funcionario['nome'] # Ajuste o nome do campo conforme a resposta da API
end

require_relative '~/apiFuncionarios/funcionarios/config/environments'
class Funcionario
